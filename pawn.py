...
class Pawn(Piece):
    ...
    def moves(self, node):
            ...
            if self.x > 0 and (piece := node.board[newx + self.y * 8]) != None and piece.is_white != self.is_white:
                if isinstance(piece, Pawn):
                    if piece.enpassant_rule_active:
                        yield Move(Pawn(newx, newy, self.is_white), (self.x, self.y), (newx, newy), enpassant=True)
        ...
        if (self.is_white and self.y == 6) or (not self.is_white and self.y == 1):
            ...
            newx, newy = self.x, self.y + 1 * direction
            if node.board[newx + newy*8] == None:
                yield Move(Knight(newx, newy, self.is_white), (self.x, self.y), (newx, newy))
                yield Move(Queen(newx, newy, self.is_white), (self.x, self.y), (newx, newy))
                yield Move(Rook(newx, newy, self.is_white), (self.x, self.y), (newx, newy))
                yield Move(Bishop(newx, newy, self.is_white), (self.x, self.y), (newx, newy))
